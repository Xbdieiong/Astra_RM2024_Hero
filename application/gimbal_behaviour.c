...
static void gimbal_chassis_divided_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set);
...
void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    //根据云台行为状态机设置电机状态机
    if (gimbal_behaviour == GIMBAL_ZERO_FORCE)  //无力
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_RAW;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_RAW;
    }
    else if (gimbal_behaviour == GIMBAL_INIT)  //初始化
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
    }
    else if (gimbal_behaviour == GIMBAL_CALI)  //校准
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_RAW;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_RAW;
    }
    ...
    else if (gimbal_behaviour == GIMBAL_RELATIVE_ANGLE)  //相对角度
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
    }
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)   //云台不动
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
    }
		...
		{
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO_XTL;       //yaw轴绝对角度控制
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;  //pitch轴相对角度控制
		}			
		else if(gimbal_behaviour == GIMBAL_CHASSIS_DIVIDED)
		{
			  gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;       //yaw轴绝对角度控制
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;  //pitch轴相对角度控制
		}
...
void gimbal_behaviour_control_set(fp32 *add_yaw, fp32 *add_pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
		...
		{
			  gimbal_chassis_divided_control(add_yaw, add_pitch, gimbal_control_set);
		}
...
bool_t gimbal_cmd_to_chassis_stop(void)   //单独调试底盘时需注掉此部分，正常情况不能注，否则初始化时底盘会一直转
{
...
//一键掉头模式
int escape_flag = 0;
int escape_open_flag = 0;
int last_escape_open_flag = 0;
...
static void gimbal_behavour_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    if (gimbal_behaviour == GIMBAL_INIT)
    {
		...
		//小陀螺状态机设置（按键开关）
//				if(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_G)//检测到陀螺键是否按下   
//				{
//					xtl_open_flag = 1;//陀螺键按下为1 弹起为0
//				}
//				else xtl_open_flag = 0;
//				
//				if(xtl_open_flag && (!last_xtl_open_flag))//last_xtl_open_flag： 长按判断 以前是否按过陀螺键 初始化赋值为0
//				{
//					xtl_key_count++;  //按键计次+1  初始化赋值为0
//				}
//				
//				if((xtl_key_count)%2) //除2余数为1就进陀螺 否则关陀螺   count=1 
//				{
//					xtl_flag = 1;
//				}
//				else
...
//					xtl_flag = 0;
//				}
//				last_xtl_open_flag = xtl_open_flag;// 历史按键记录 防止长按不松手导致多次计数 按键弹起后归零（状态机复位）
//				
//				if(get_remain_HP() == 0)//死亡时重新初始化
//				{
//					xtl_flag = 0;  				//关陀螺
//					//xtl_key_count = 0;		//关陀螺计数
//					
//				}
       ...
       if(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_C)
				 {
				  	xtl_flag = 1;
				 }
				 else
		...
		//狙击模式状态机（按住起效）
				if(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_Q)//检测狙击键是否按下   
				{
					sniper_flag = 1;//狙击模式键按下为1 弹起为0
				}
				else sniper_flag = 0;
		//一键掉头
					
				if(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_F)
				{	
					escape_open_flag = 1;
				}
				else
				...
				if(escape_open_flag && !(last_escape_open_flag))
				{
					if(escape_flag == 0)
						escape_flag = 1;
				}
				last_escape_open_flag = escape_open_flag;
...
//				
    ...
    //开关控制 云台状态
    if (switch_is_down(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]))  //右拨杆下档（急停）
    {
        gimbal_behaviour = GIMBAL_ZERO_FORCE;  //无力模式
    }
    else if (switch_is_mid(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]))  //右拨杆中档（正常操控）
    {
				if(xtl_flag == 1 ) //|| windmill_mode == 1 || windmill_mode == 2)
				{
					gimbal_behaviour = GIMBAL_XTL;     //绝对角度，小陀螺
				}
				else
				{
//					if(rotate_flag == 1)
//					{
						gimbal_behaviour = GIMBAL_ABSOLUTE_ANGLE;  //绝对角度，底盘跟随云台
//					}
//					else if(rotate_flag == 0)
//					{
//						gimbal_behaviour = GIMBAL_RELATIVE_ANGLE;  //相对角度，底盘云台分离
//					}
				}
	  }
    else if (switch_is_up(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]))  //右拨杆上档（强制小陀螺）
    {		
					gimbal_behaviour = GIMBAL_XTL;     //绝对角度，小陀螺
//			gimbal_behaviour = GIMBAL_RELATIVE_ANGLE;
    ...
    {
        gimbal_behaviour = GIMBAL_ZERO_FORCE;   //遥控器未连接上，云台无力模式
    }
    ...
    //判断进入init状态机
    {
        ...
        {
          gimbal_behaviour = GIMBAL_INIT;          //每次从无力切换到其它状态都要进行初始化
        }
...
static void gimbal_absolute_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    if((autoaim_mode_flag == 0)||(CameraDetectTarget_Flag == 0))  //正常操作手模式
	{
      ...
      rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[PITCH_CHANNEL], pitch_channel, RC_DEADBAND);
			...
			//遥控器控制  +  鼠标控制   注意正负号
      *yaw = (yaw_channel * YAW_RC_SEN - gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN)/2.0f;
	  *pitch = (pitch_channel * PITCH_RC_SEN + gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN)/5.0f;
	...
	if(autoaim_mode_flag == 1)  //自瞄
		{
			...
			count_Sendtosight++;
			//gimbal_control_set ->gimbal_yaw_motor.absolute_angle_set = gimbal_control_set ->gimbal_yaw_motor.absolute_angle_set-(gimbal_control_set ->gimbal_yaw_motor.autoaim_angle)/100;// + (ChariotRecognition_yaw - last_ChariotRecognition_yaw)/20 * count_Sendtosight;
			//gimbal_control_set ->gimbal_pitch_motor.relative_angle_set = gimbal_control_set ->gimbal_pitch_motor.relative_angle_set-(gimbal_control_set ->gimbal_pitch_motor.autoaim_angle/100);// + (ChariotRecognition_pitch - last_ChariotRecognition_pitch)/20 * count_Sendtosight;
			//gimbal_control_set ->gimbal_yaw_motor.absolute_ngle_set =(gimbal_control_set ->gimbal_yaw_motor.autoaim_angle/1);
			*yaw = (visionDataGet.yaw_angle.yaw_predict/65000.0*PI/180);
			*pitch = (visionDataGet.pitch_angle.pitch_predict/65000.0*PI/180);	
		}
...
static void gimbal_relative_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    if (yaw == NULL || pitch == NULL || gimbal_control_set == NULL)
    {
      ...
      static int16_t yaw_channel = 0, pitch_channel = 0;
			rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[YAW_CHANNEL], yaw_channel, RC_DEADBAND);
			rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[PITCH_CHANNEL], pitch_channel, RC_DEADBAND);
			*yaw = (yaw_channel * YAW_RC_SEN - gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN)/5.0f;
			*pitch = (pitch_channel * PITCH_RC_SEN + gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN)/5.0f;
		}
		else if((autoaim_mode_flag == 1)&&CameraDetectTarget_Flag)    //自瞄
		{

			count_Sendtosight++;
			gimbal_control_set ->gimbal_yaw_motor.absolute_angle_set = gimbal_control_set ->gimbal_yaw_motor.absolute_angle_set+(gimbal_control_set ->gimbal_yaw_motor.autoaim_angle*1);// + (ChariotRecognition_yaw - last_ChariotRecognition_yaw)/20 * count_Sendtosight;
			gimbal_control_set ->gimbal_pitch_motor.relative_angle_set = gimbal_control_set ->gimbal_pitch_motor.relative_angle_set-(gimbal_control_set ->gimbal_pitch_motor.autoaim_angle*1);// + (ChariotRecognition_pitch - last_ChariotRecognition_pitch)/20 * count_Sendtosight;
			//gimbal_control_set ->gimbal_yaw_motor.absolute_angle_set = (gimbal_control_set ->gimbal_yaw_motor.autoaim_angle/1);
			//gimbal_control_set ->gimbal_pitch_motor.relative_angle_set = (gimbal_control_set ->gimbal_pitch_motor.autoaim_angle/1);
		}	
}
