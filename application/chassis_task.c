...
static void chassis_rotate_judge(chassis_move_t *chassis_rotate);
...
/**
  ...
  */
/**
  * @brief          设置底盘控制设定值，三种移动控制值由"chassis_behaviour_control_set"函数设定
  * @param[out]     chassis_move_update:"chassis_move"变量指针.
  * @retval         none
  */
...
extern void Sendtosightway(int value);
//狙击模式下底盘xyz的set值与实际数据更新全部归零
extern int sniper_flag;//狙击模式标志位
...
void chassis_task(void const *pvParameters)
{
    ...
    //判断底盘电机是否都在线
//    while (toe_is_error(CHASSIS_MOTOR1_TOE)|| toe_is_error(CHASSIS_MOTOR2_TOE) || toe_is_error(CHASSIS_MOTOR3_TOE) || toe_is_error(CHASSIS_MOTOR4_TOE) || toe_is_error(DBUS_TOE))// || toe_is_error(CHASSIS_MOTOR2_TOE) || toe_is_error(CHASSIS_MOTOR3_TOE) || toe_is_error(CHASSIS_MOTOR4_TOE) || toe_is_error(DBUS_TOE))
//    {
...
//        Sendtosightway(16);
			  //前后左右速度过渡
			  start_run();
			  //3、4、5号步兵判断（代码合并用） 后续可改为工程飞镖复用代码
				controller_judge();
				//super cape send & resive
			
			//每10ms发电压（100HZ），2ms发触发信号读电压（500HZ） 循环0.4ms 1次（4000hz）雾列改为10hz发送和接收（100ms） 暂测250个循环收发一次
				//同时收发这里只保留了一个判断
				if(time_count%10 == 0)
				{
					//雾列功控板发：
					super_cape_power_set ( get_chassis_power_limit()*100);
					//500块功控板发：
					 //pm01_access_poll();
					//雾列功控板收（设置攻率 当前电压）：
					
					//500块功控板收（发送电压 功率设置 当前电压）：
					//pm01_voltage_set( 2500, 0);
					//pm01_power_set( get_chassis_power_limit()*100 , 0 );
					//pm01_current_set( 800 , 1 );
//					CAN_cmd_supercap_voltage(2500);
//					CAN_cmd_supercap_power(capacitor_power*10);
				}
				
			  //旋转判断
			  chassis_rotate_judge(&chassis_move);
        ...
        if (!(toe_is_error(CHASSIS_MOTOR1_TOE) && toe_is_error(CHASSIS_MOTOR2_TOE) && toe_is_error(CHASSIS_MOTOR3_TOE) && toe_is_error(CHASSIS_MOTOR4_TOE)))
        {
            ...
            {   
                ...
                //发送控制电流
//							  CAN_cmd_chassis(0, 0, 0, 0);      //需要注掉底盘运动时给底盘电机赋值0电流
							if(switch_is_down(chassis_move.chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))
							{
								 CAN_cmd_chassis(0, 0, 0, 0);  
							}
							else
...
static void chassis_init(chassis_move_t *chassis_move_init)
{
    ...
    uint8_t i;
		...
		pm01_cmd_send(2, 1);
		pm01_current_set( 800 , 1 );
		pm01_voltage_set( 2500, 1 );
		pm01_power_set( 4000 , 1 ); 
...
int id_of_controller;
//机器人ID匹配函数 纯英雄代码暂时无用
void controller_judge(void)
{
		if(get_robot_id() == 1||get_robot_id() == 101)
	{
		id_of_controller = 1;
	} 
	else if(get_robot_id() == 3||get_robot_id() == 103)
	{
		id_of_controller = 3;
	} 
	else if(get_robot_id() == 4||get_robot_id() == 104)  
	{
		id_of_controller = 4;
	}
	else if(get_robot_id() == 5 || get_robot_id() == 105)
	{
		id_of_controller = 5;
	}
	else
...
static void chassis_rotate_judge(chassis_move_t *chassis_rotate)
{
	...
	{
		if(!( chassis_rotate->wz_set == 0 && (rc_ctrl.mouse.x == 0) && (rc_ctrl.mouse.y == 0)
			&&!(chassis_rotate->chassis_RC->key.v & KEY_PRESSED_OFFSET_W)
			&&!(chassis_rotate->chassis_RC->key.v & KEY_PRESSED_OFFSET_S)
			&&!(chassis_rotate->chassis_RC->key.v & KEY_PRESSED_OFFSET_A)
			&&!(chassis_rotate->chassis_RC->key.v & KEY_PRESSED_OFFSET_D)	
			))
		{
			rotate_flag = 1;//如果发送的旋转速度不为0 且没有平移就令旋转标志位为1 
		}
...
//二次函数比例系数
float START_RUN_SEN_X;// = 0.000012 * ((int)get_robot_level()+ 1);    //0.0048;//0.000016  //前后     //灵敏度 * 250 * 250 就是速度
float START_RUN_SEN_Y;// = 0.000012;   //0.000012  //左右
extern int flying_flag = 0;
...
static void start_run()
{
	...
	if(get_game_type() == 2)  //1V1
	{
		START_RUN_SEN_X = 0.000015 * 3;  //*2.5 1V1速度，同3V3中3级步兵    // * 1.5 1V1速度，同3V3中1级步兵 //*3
		START_RUN_SEN_Y = 0.000015 * 3;		
	}
	else
	{
		if((flying_flag)&&(xtl_flag != 1))
		{
		...
		else
		{
			...
			{
				START_RUN_SEN_X = 0.000015 * 0.5 *(get_robot_level()+ 2);  //初始  1级乘1.5   2级乘2   3级乘2.5
				START_RUN_SEN_Y = 0.000015 * 0.5 *(get_robot_level()+ 2);
			}
		}
	}
...
/**
  ...
  * @param[out]     vx_set: ??????
  * @param[out]     vy_set: ??????
  * @param[out]     chassis_move_rc_to_vector: "chassis_move" ????
  * @retval         none
  */
...
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    {
			    vx_set_channel = start_run_speed_x*150/100;  //前115
//			    vy_set_channel = 0;//强制走直线横移输出为零
//        vx_set_channel = chassis_move_rc_to_vector->vx_max_speed;
    ...
    {
			    vx_set_channel = -start_run_speed_x*150/100;  //后115
//        vx_set_channel = chassis_move_rc_to_vector->vx_min_speed;
    ...
    {
			    vy_set_channel = start_run_speed_y*150/100;  //左115
//        vy_set_channel = chassis_move_rc_to_vector->vy_max_speed;
    ...
    {
			    vy_set_channel = -start_run_speed_y*150/100;  //右115
//        vy_set_channel = chassis_move_rc_to_vector->vy_min_speed;
    ...
    {
			    vx_set_channel = start_run_speed_x/100*120;  //前80
			    vy_set_channel = start_run_speed_y/100*120;  //左
//        vx_set_channel = chassis_move_rc_to_vector->vx_max_speed;
    }
    ...
    {
			    vx_set_channel = -start_run_speed_x/100*120;  //后80
			    vy_set_channel = start_run_speed_y/100*120;  //左
//        vx_set_channel = chassis_move_rc_to_vector->vx_min_speed;
    }
    ...
    {
					vx_set_channel = start_run_speed_x/100*120;  //前80
			    vy_set_channel = -start_run_speed_y/100*120;  //右
//        vy_set_channel = chassis_move_rc_to_vector->vy_max_speed;
    }
...
static void chassis_vector_to_mecanum_wheel_speed(const fp32 vx_set, const fp32 vy_set, const fp32 wz_set, fp32 wheel_speed[4])
{
	  ...
	  if(chassis_behaviour_mode == CHASSIS_XTL)//陀螺移动如果姿势不对改这里
		{
			//新姿态解算： 姿势正确了
			//3 4 
      wheel_speed[0] = + (vx_set * AHRS_cosf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 ...
			                 + (vx_set * AHRS_sinf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 + (CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
			//1 3
			wheel_speed[1] = - (vx_set * AHRS_cosf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 - (vy_set * AHRS_sinf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 - (vy_set * AHRS_cosf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 + (vx_set * AHRS_sinf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 + (CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
			//3 4
			wheel_speed[2] = - (vx_set * AHRS_cosf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 - (vy_set * AHRS_sinf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 + (vy_set * AHRS_cosf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 - (vx_set * AHRS_sinf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 + (CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
			//1 3
			wheel_speed[3] = + (vx_set * AHRS_cosf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 + (vy_set * AHRS_sinf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 + (vy_set * AHRS_cosf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 - (vx_set * AHRS_sinf(-gimbal_control.gimbal_yaw_motor.relative_angle))\
			                 + (CHASSIS_WZ_SET_SCALE - 1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
...
static void chassis_control_loop(chassis_move_t *chassis_move_control_loop)
{
    ...
    for (i = 0; i < 4; i++)
    {
			  chassis_move.motor_speed_pid[i].Kp = 15000;   		//15000
			  chassis_move.motor_speed_pid[i].Ki = 0;      //0//10
			  chassis_move.motor_speed_pid[i].Kd = 20000;  //20000    //0
			  if(escape_flag == 1)
			  {
				  chassis_move.chassis_angle_pid.Kp = 10;        //15   
				  chassis_move.chassis_angle_pid.Ki = 0.01;     //0.001       
				  chassis_move.chassis_angle_pid.Kd = 0;      //30  				
			  }
			  else
			  {
				  chassis_move.chassis_angle_pid.Kp = 10;   	//10     //15   
				  chassis_move.chassis_angle_pid.Ki = 0.015; 	//0.03    //0.001       
				  chassis_move.chassis_angle_pid.Kd = 0;     	//0 //30  				
			  }
    ...
    //功率控制 ,只有在1V1和3V3的时候才用功率控制
//	if(get_game_type() == 1 || get_game_type() == 5)
		{
