...
/**
  ...
  */
/**
  * @brief          云台控制模式:GIMBAL_MOTOR_GYRO，使用陀螺仪计算的欧拉角进行控制（小陀螺）
  * @param[out]     gimbal_motor:yaw电机或者pitch电机
  * @retval         none
  */
...
void gimbal_task(void const *pvParameters)
{
	...
	/*----------pitch轴和yaw轴电机中值及最大最小角度限制----------*/
	//删除判断兵种编号进行中值设定
	//英雄pitch轴中值4220 yaw轴中值2777
...
//			  yaw_ins_offset += 4.89e-6;
			  ...
			  windmill_mode_change(&gimbal_control);//自瞄控制        
...
static void gimbal_feedback_update(gimbal_control_t *feedback_update)
{
    ...
    //云台数据更新 绝对角改动：PITCH轴更改为ROLL轴并取负数 仰角为正 俯角为负
...
static void gimbal_mode_change_control_transit(gimbal_control_t *gimbal_mode_change)
{
    ...
    {
        gimbal_mode_change->gimbal_yaw_motor.absolute_angle_set = gimbal_mode_change->gimbal_yaw_motor.absolute_angle;//陀螺模式
    }
...
static void gimbal_absolute_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add)
{
    ...
    angle_set = gimbal_motor->absolute_angle_set;
		...
		if(sniper_flag == 1 )
		{
      gimbal_motor->absolute_angle_set = rad_format(angle_set + add/10);//狙击模式下增量减小(原100)
	  }
...
static void gimbal_relative_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add)
{
    if (gimbal_motor == NULL)
    {
		...
		if(sniper_flag == 1)
		{
      gimbal_motor->relative_angle_set += add/10;//狙击模式下增量减小(原100)
	  }
		else if((autoaim_mode_flag == 0)||(CameraDetectTarget_Flag == 0))
		{
      gimbal_motor->relative_angle_set += add;
	  }
	  else if((autoaim_mode_flag == 1)&& CameraDetectTarget_Flag)  //自瞄模式
	  {
		  gimbal_motor->relative_angle_set += add;  //pitch轴直接赋值给ref，不用角度增量
	  }
...
static void gimbal_control_loop(gimbal_control_t *control_loop)
{
    ...
    {
        gimbal_motor_absolute_angle_control(&control_loop->gimbal_yaw_motor);     //yaw轴绝对角度控制
    }
	else if (control_loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_GYRO_XTL)
    {
        gimbal_motor_absolute_angle_control_xtl(&control_loop->gimbal_yaw_motor);     //yaw轴绝对角度控制（小陀螺）
    }
    ...
    {
        gimbal_motor_relative_angle_control(&control_loop->gimbal_pitch_motor);   //pitch轴相对角度控制
    }
}
...
static void gimbal_motor_absolute_angle_control(gimbal_motor_t *gimbal_motor)
{
    if (gimbal_motor == NULL)
    {
      ...
      gimbal_motor->given_current = (int16_t)(gimbal_motor->current_set);
		...
		else if(autoaim_mode_flag == 1)   //自瞄
		{
...
/**
  ...
  */
/**
  * @brief          云台控制模式:GIMBAL_MOTOR_GYRO_XTL，使用陀螺仪计算的欧拉角进行控制（小陀螺）
  * @param[out]     gimbal_motor:yaw电机或者pitch电机
  * @retval         none
  */
static void gimbal_motor_absolute_angle_control_xtl(gimbal_motor_t *gimbal_motor)
...
static fp32 pitch_PID_calc(gimbal_PID_t *pid, fp32 get, fp32 set, fp32 error_delta)
{
    ...
    pid->err = rad_format(err);
		...
		pid->Pout = pid->kp *(AHRS_cosf(get))*(pid->err);//在pid基础上叠加重力矩函数
