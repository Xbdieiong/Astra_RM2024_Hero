...
static void chassis_gimbal_divided_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector);
...
//旋转标志变量
int rotate_flag = 0;
int last_rotate_flag = 0;
int change_servo = 0;  //舵机运动标志
...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    //遥控器设置模式
    if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))     //右拨杆中档（正常操控）
    {
			//flying_flag=1;//遥控器飞坡测试
...
//			  else if(rotate_flag == 0)
				...
				if(autoaim_mode_flag==1) //自瞄模式底盘分离
				{
					chassis_behaviour_mode = CHASSIS_GIMBAL_DIVIDED;
				}
				/*
			  if( sniper_flag==1)  //狙击模式底盘分离
				{
					chassis_behaviour_mode = CHASSIS_GIMBAL_DIVIDED;
				}
				*/
    ...
    else if (switch_is_down(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))   //右拨杆下档（急停）
    {
        chassis_behaviour_mode = CHASSIS_ZERO_FORCE;   //急停模式，底盘不动
			  autoaim_mode_flag = 0;      //关闭自瞄模式
//        servo_pwm_set(500, 3);     //关闭弹仓
//			  change_servo = 0;   //不能通过按键改变舵机位置
			 shoot_speed_add_little = 0;
    }
    else if (switch_is_up(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))    //右拨杆上档（强制小陀螺）
    {
//							chassis_behaviour_mode = CHASSIS_XTL;     
    ...
    {
        chassis_behaviour_mode = CHASSIS_NO_MOVE;    //如果要使用底盘云台分离模式单独测试底盘，需要注掉这几句
    }
    ...
    else if (chassis_behaviour_mode == CHASSIS_NO_MOVE)   //底盘不动
    {
    ...
    else if (chassis_behaviour_mode == CHASSIS_ENGINEER_FOLLOW_CHASSIS_YAW)  //底盘跟随底盘，工程用
    {
        chassis_move_mode->chassis_mode = CHASSIS_VECTOR_FOLLOW_CHASSIS_YAW;
    }
    else if (chassis_behaviour_mode == CHASSIS_NO_FOLLOW_YAW)  //底盘不跟随云台，底盘云台分离
    {
        chassis_move_mode->chassis_mode = CHASSIS_VECTOR_NO_FOLLOW_YAW;
    }
    else if (chassis_behaviour_mode == CHASSIS_OPEN)   //开环
    {
        chassis_move_mode->chassis_mode = CHASSIS_VECTOR_RAW;
    }
		...
		else if (chassis_behaviour_mode == CHASSIS_GIMBAL_DIVIDED)
		{
			  chassis_move_mode->chassis_mode = CHASSIS_VECTOR_NO_FOLLOW_YAW;
		}			
...
void chassis_behaviour_control_set(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    else if (chassis_behaviour_mode == CHASSIS_OPEN)
    {
		...
		{
			  chassis_gimbal_divided_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
		}
}
...
static void chassis_xtl_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    if(AHRS_sinf((fp32)(xtl_time * 3.14159 / 180)) <= -0.1)   //正弦函数下限-0.1，可改
	  ...
	  chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);   //控制底盘前后左右
		if(get_game_type() == 5)  //1V1
		{
			*wz_set = (float)(6.5 + (0.8/360 * xtl_time + 1) * changing_speed_function);    //10.5    6.5
		}
		else
		{
			*wz_set = -((float)(3.5 + 1 * (get_robot_level()))); //常规匀速小陀螺·
	    //*wz_set = -((float)(3.5 + 1 * (get_robot_level()) + (0.8/360 * xtl_time + 1) * changing_speed_function));   //底盘旋转速度（周期性变速 云台会很抖）
		  //            等级系数     等级         系数（1~1.8随时间线性变化）* sin函数 （角度转弧度）
		  //                      0，1，2，3      （ ？）/360 * xtl_time + （~） ,其中 “ ？” 等于区间长度, ” ~ ” 等于区间下限
		}
