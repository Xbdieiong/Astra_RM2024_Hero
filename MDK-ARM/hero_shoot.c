...
//			 }
    ...
    else  //非射击停止模式
    {
			...
			{
				if(id_of_controller == 3 )
				{
					shoot_control.fric1_speed_set = -(1.60 + KEY_TO_FRIC_SPEED_SEN * shoot_speed_add_little);
					shoot_control.fric2_speed_set = 1.60 + KEY_TO_FRIC_SPEED_SEN * shoot_speed_add_little;
				}
				else if(id_of_controller == 4)      
				{
					shoot_control.fric1_speed_set = -(1.63 + KEY_TO_FRIC_SPEED_SEN * shoot_speed_add_little);
					shoot_control.fric2_speed_set = 1.63 + KEY_TO_FRIC_SPEED_SEN * shoot_speed_add_little;
				}
				else if(id_of_controller == 5)
				{
					shoot_control.fric1_speed_set = -(1.64 + KEY_TO_FRIC_SPEED_SEN * shoot_speed_add_little);
					shoot_control.fric2_speed_set = 1.64 + KEY_TO_FRIC_SPEED_SEN * shoot_speed_add_little;
				}
			}
...
//    static int8_t last_s = RC_SW_UP;
    ...
    else if(shoot_control.shoot_mode == SHOOT_READY)
    {
        ...
        if ( (switch_is_up(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]) ) || (shoot_control.press_l && shoot_control.last_press_l == 0))  //|| (shoot_control.press_l && shoot_control.last_press_l == 0) 
        {
					if(get_game_progress() == 4)
					{
						get_shoot_heat0_limit_and_heat0(&shoot_control.heat_limit, &shoot_control.heat);

						if(id_of_controller == 3||id_of_controller == 4)
						{
    ...
    if(shoot_control.shoot_mode > SHOOT_READY)
    {
        ...
        else if(( !(shoot_control.press_l)) && switch_is_mid(shoot_control.shoot_rc->rc.s[SHOOT_RC_MODE_CHANNEL]))  //!(shoot_control.press_l) &&
        {
            shoot_control.shoot_mode = SHOOT_READY;
        }
    }
